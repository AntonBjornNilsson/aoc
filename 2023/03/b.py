    ...
    for i, numbers in enumerate(numbers_line):
        for number in numbers:
            ...
            for line in range(max(0, i-1), min(i+2, len(example))):
                for col in range(max(0, num_index-1), min(num_index+len(num_value) + 1, len(example[i]))):
                    char = example[line][col]
                    if char == "*":
                        gear_coord = (line, col)
                _num_str = "".join([example[line][col] for col in range(max(0, num_index-1), min(num_index+len(num_value) + 1, len(example[i])))])
                num_str += _num_str
